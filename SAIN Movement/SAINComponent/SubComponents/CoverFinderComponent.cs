    ...
    public class CoverFinderComponent : MonoBehaviour
    {
        ...
        private IEnumerator FindCover()
        {
            while (true)
            {
                ...
                float minDist = GetMinimumDistance(originPoint, targetPosition);
            ...
            else
            {
                if (NavMesh.SamplePosition(collider.Collider.transform.position - (collider.TargetPosition - collider.NavMeshHit.position).normalized * 2, out NavMeshHit hit2, navSampleSize, NavMesh.AllAreas))
                {
                    ...
                    if (NavMesh.CalculatePath(SAIN.BotOwner.Transform.position, hit2.position, NavMesh.AllAreas, Path) && Path.status == NavMeshPathStatus.PathComplete)
                    {
                        ...
                        if (pathLength <= maxDistance * MaxPathLengthMulti)
                        {
        ...
        private class ColliderArray : SAIN.SAINBot
        {
                ...
                if (CheckColliderTimer < Time.time)
                {
                    ...
                    for (int i = 0; i < hits; i++)
                    {
                        ...
                        if (!badCollider)
                        {
                            ...
                            bool behindEnemy = ColliderChecks.DotProductCheck(inputParams.OriginPoint.Value, inputParams.TargetPosition.Value, colliderPos);
                        ...
                        if (!badCollider)
                        {
                            if (ColliderChecks.CheckPathLength(inputParams.OriginPoint.Value, Colliders[i], inputParams.MaxRange, out ColliderWithPath point))
                            {
        ...
        private class ColliderChecks
        {
            ...
            {
                if (NavMesh.SamplePosition(collider.transform.position, out var hit, 3f, -1))
                {
                    NavMeshPath Path = new NavMeshPath();
                    if (NavMesh.CalculatePath(botPos, hit.position, NavMesh.AllAreas, Path) && Path.status == NavMeshPathStatus.PathComplete)
                    {
                        float pathLength = Path.CalculatePathLength();
