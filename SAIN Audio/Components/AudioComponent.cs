...
namespace SAIN_Movement.Components
{
    public class AudioComponent : MonoBehaviour
    {
        private void Awake()
        {
            ...
            Logger = BepInEx.Logging.Logger.CreateLogSource(nameof(AudioComponent));
        }
        ...
        private void ReactToSound(IAIDetails person, Vector3 pos, float power, bool wasHeard, AISoundType type)
        {
            ...
            if (wasHeard)
            {
                ...
                var placeForCheck = BotOwner.BotsGroup.AddPointToSearch(vector, power, BotOwner);
                ...
                if (person != null && isGunSound)
                {
                    ...
                    if (soundclose && firedAtMe)
                    {
                        ...
                        if (shooterDistance > 80f) BotOwner.BotTalk.Say(EPhraseTrigger.SniperPhrase, false, null);
                    }
                }
            ...
            else if (person != null && isGunSound && bulletfelt)
            {
                ...
                if (firedAtMe && soundclose)
                {
                    ...
                    var placeForCheck2 = BotOwner.BotsGroup.AddPointToSearch(estimate, 50f, BotOwner);
