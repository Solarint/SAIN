...
namespace SAIN.Helpers
{
    public class Corners
    {
        ...
        public class FindDirectionToLean
        {
            ...
            {
                // Corner 0 is at BotOwner position. So we need 3 RawCorners to check lean angle.
                if (corners.Length < 3)
                {
            ...
            /// <param name="botPosition">The BotOwner's position.</param>
            private void DebugDrawMode(Vector3 firstcorner, Vector3 secondcorner)
            {
                if (DebugTimer < Time.time)
                {
                    ...
                    SAIN_Helpers.DebugDrawer.Ray(BotHeadPosition, firstcorner - BotHeadPosition, Vector3.Distance(BotHeadPosition, firstcorner), 0.025f, Color.red, 5f);
                    SAIN_Helpers.DebugDrawer.Ray(BotHeadPosition, secondcorner - BotHeadPosition, Vector3.Distance(BotHeadPosition, secondcorner), 0.025f, Color.white, 5f);

                    SAIN_Helpers.DebugDrawer.Ray(firstcorner, Vector3.down, 1.3f, 0.025f, Color.red, 5f);
                    SAIN_Helpers.DebugDrawer.Ray(secondcorner, Vector3.down, 1.3f, 0.025f, Color.red, 5f);
                }
            }
        ...
        public class CornerProcessing
        {
            ...
            /// <summary>
            /// Calculates the RawCorners of the navMeshPath between the BotOwner and the target position, and optionally trims them using raycasting, length trimming, and lerp trimming.
            /// </summary>
            ...
            /// <param name="cornerToHeadHeight">Whether to raise the RawCorners to the BotOwner's head height.</param>
            ...
            public Vector3[] GetCorners(Vector3 botPosition, Vector3 targetPos, Vector3 botHeadPosition, bool raycastTrim = true, bool lengthTrim = false, bool lerpTrim = false, bool cornerToHeadHeight = false, bool returnFromHeadHeight = false, float minlengthfortrim = 2f)
            {
                ...
                if (cornerToHeadHeight)
                {
                    ...
                    for (int i = 0; i < NavmeshCornersList.Count; i++)
                    {
                        ...
                        if (DebugMode)
                        {
                            SAIN_Helpers.DebugDrawer.Line(corner, botHeadPosition, 0.025f, Color.magenta, 0.25f);
                        }
                    }
                ...
                if (cornerToHeadHeight && returnFromHeadHeight && corners.Length > 0)
                {
                    ...
                    for (int i = 0; i < NavmeshCornersList2.Count; i++)
                    {
                        ...
                        if (DebugMode)
                        {
                            SAIN_Helpers.DebugDrawer.Line(corner2, botHeadPosition, 0.025f, Color.magenta, 0.25f);
                        }
                    }
