...
namespace SAIN.Classes
{
    public class EnemyClass : SAINBot
    {
        ...
        public void Update(IAIDetails person)
        {
            ...
            {
                CheckPathTimer = Time.time + CheckPathFreq;
                Path.Update(person.Transform.position);
            }

            if (VisionRayCastTimer < Time.time)
            {
                VisionRayCastTimer = Time.time + RayCastFreq;
                UpdateVision(person);
            }
        }

        private const float RayCastFreq = 0.05f;
        private const float CheckPathFreq = 0.25f;
        private float VisionRayCastTimer = 0f;
