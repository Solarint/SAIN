...
namespace SAIN.Components
{
    public class DecisionComponent : MonoBehaviour
    {
        ...
        {
            return !SAIN.Cover.BotIsAtCoverPoint && SAIN.HasEnemyAndCanShoot;
        }

        private bool StartSeekEnemy()
        {
            return !SAIN.HasEnemyAndCanShoot;
            // && Enemy.LastSeen.TimeSinceSeen > TimeBeforeSearch
        }
        ...
        private bool StartSurgery()
        {
            ...
            if (BotShouldHeal)
            {
                ...
                else
                {
                    ...
                    string enemydistReason = DebugClass.Reason(SAIN.Enemy);
        ...
        private void DebugDrawPath(float enemyDistance, float minDistance, bool active)
        {
            if (DebugMode && DebugTimer < Time.time && enemyDistance < minDistance)
            {
                ...
                SAIN.DebugDrawList.DrawTempPath(SAIN.Enemy.Path.Path, active, Color.red, Color.green, 0.1f, 1f, true);
            }
        }
